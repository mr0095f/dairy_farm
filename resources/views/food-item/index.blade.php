...
<section class="content-header">
  <h1><i class="icon-list"></i> {{__('food_item.title') }}</h1>
  <ol class="breadcrumb">
...
<section class="content">
  ...
  <div class="box box-success">
    <div class="box-header with-border" align="right">
      <a href="#addModal" class="btn btn-success btn-sm" data-toggle="modal">
        <i class="fa fa-plus-square"></i> <b>{{__('same.add_new') }}</b>
      </a> 
      <a href="{{URL::to('food-item')}}" class="btn btn-warning btn-sm">
        <i class="fa fa-refresh"></i> <b>{{__('same.refresh') }}</b>
      </a> 
    </div>
                    ...
                    <td>
                      <div class="form-inline">
                          <div class = "input-group">
                            <a href="#editModal{{$data->id}}" class="btn btn-success btn-xs" data-toggle="modal" title="{{__('same.edit') }}"><i class="icon-pencil"></i></a>
                          </div>
                          
                          <div class = "input-group"> 
                            {{Form::open(array('route'=>['food-item.destroy',$data->id],'method'=>'DELETE'))}}
                          ...
                          <div class="modal-dialog modal-sm">
                            <div class="modal-content">
                              <div class="modal-header">
                                ...
                                <h4 class="modal-title"><i class="fa fa-edit edit-color"></i> {{__('food_item.update_food_unit') }}</h4>
                              </div>
                                {!! Form::open(array('route' => ['food-item.update', $data->id],'class'=>'form-horizontal','method'=>'PUT')) !!}
                              <div class="modal-body">
                                <div class="form-group">
                                    <div class="col-md-12">
                                      <label for="name">{{__('food_item.name') }} <span class="validate">*</span> : </label>
                                      <input type="text" class="form-control" value="{{  $data->name  }}" name="name" placeholder="{{__('food_item.name') }}" required>
                                    </div>
                                </div>
