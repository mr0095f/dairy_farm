...
@section('title', __('manage_cow.calf_information'))
@section('content')  
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="fa fa-paw"></i> {{__('manage_cow.calf_information') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('manage_cow.calf_information') }}</li
  </ol>
</section>
<section class="content">
  ...
  <div class="box box-success">
      ...
      {{  Form::open(array('route' => 'calf.store', 'method' => 'post', 'files' => true))  }}
      <?php $btn_name = __('same.save'); ?>
    @else
      {{  Form::open(array('route' => ['calf.update',$single_data->id], 'method' => 'PUT', 'files' => true))  }}
    ...
    <div class="box-header with-border" align="right">
		...
		<a href="{{  url('calf/create')  }}" class="btn btn-warning btn-sm"><i class="fa fa-refresh"></i> <b> {{__('same.refresh') }}</b></a>
		&nbsp;&nbsp; 	
	</div>
   	<div class="box-body">
	    <div class="col-md-12">
		    	...
		    	<div class="panel-body">
		    		<div class="row">
		    			<div class="col-md-12"> 
							...
							<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="animal_id">{{__('manage_cow.mother_id') }} <span class="validate">*</span> : </label>
							    	<select class="form-control" name="animal_id" required>
							    		<option value="">{{__('same.select') }}</option>
							    		@foreach($mother_animals as $mother_animalData)
								    	<option value="{{$mother_animalData->id}}" {{(!empty($single_data->animal_id))?($single_data->animal_id==$mother_animalData->id)?'selected':'':''}}>
								    		000{{$mother_animalData->id}}
								    	</option>
								    	@endforeach
								    </select>
							   	</div>
		    				...
		    				<div class="col-md-4"> 
								<div class="form-group">
							    	...
							    	<div class="input-group">
										...
										<span class="input-group-addon animal-month">{{(!empty($single_data->age))?number_format((float)$single_data->age / 30,2,".","."):'0 '}} {{__('manage_cow.month') }}</span>
									</div>
							   	</div>
							</div>
							<div class="col-md-4"> 
							...
							<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="buy_from">{{__('manage_cow.buy_form') }} <span class="validate">*</span> : </label>
							    	<input type="text" name="buy_from" class="form-control" value="{{(!empty($single_data->buy_from))?$single_data->buy_from:''}}">
							   	</div>
							</div>
							<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="buying_price">{{__('manage_cow.buying_price') }} : </label>
							    	<input type="text" name="buying_price" class="form-control" value="{{(!empty($single_data->buying_price))?$single_data->buying_price:''}}">
							   	</div>
							</div>
							<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="buy_date">{{__('manage_cow.buy_date') }} : </label>
							    	<input type="text" name="buy_date" class="form-control wsit_datepicker" value="{{(!empty($single_data->buy_date))?date('m/d/Y', strtotime($single_data->buy_date)):''}}">
							   	</div>
							</div>
							<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="shade_no">{{__('manage_cow.stall_no') }} : </label>
							    	<select class="form-control" name="shade_no">
								    	<option value="0">{{__('same.select') }}</option>
								    	@foreach($all_sheds as $sheds)
							    		<option value="{{$sheds->id}}" {{(!empty($single_data))?($sheds->id==$single_data->shade_no)?'selected':'':''}}>{{$sheds->shed_number}} <?php echo $sheds->status=='1' ? ' (Booked)' : ' (Available)'; ?></option>
							    		@endforeach
								    </select>
							   	</div>
							</div>
		...
		<div class="col-md-12">
	    	<div class="panel panel-default">
		    	...
		    	<div class="panel-body">
		    		<div class="row">
		    			<div class="col-md-12"> 
		    				@if(isset($single_data->vaccines))
		    					<?php $vaccines = json_decode($single_data->vaccines, true);?>
		    					@foreach($all_vaccines as $all_vaccine_info)
			    				<div class="col-md-4"> 
									<label class="checkbox-inline">
										<?php if(!empty($vaccines)) { ?>
										<input type="checkbox" name="vaccines[]" value="{{$all_vaccine_info->id}}" {{in_array($all_vaccine_info->id, $vaccines)?'checked':''}}>
		...
		<div class="col-md-12">
	    	<div class="panel panel-default">
				<div class="panel-heading feed-heading">
					<i class="fa fa-image"></i> {{__('manage_cow.animal_images') }}
				...
				<div class="panel-body" id="imageBlock">
					@if(isset($single_data->pictures) && !empty($single_data->pictures))
						...
						<div class="col-md-2 animal-box-height" id="div_{{$num_row}}">
					  		<div class="upload-builder-2">
						        ...
						        <div class="manage-animal-upload-2">
							        ...
							        <label class="btn btn-success btn-xs btn-file upload-builder-5">
								    	<i class="fa fa-folder-open"></i>&nbsp;&nbsp; {{__('manage_cow.upload_image') }} 
								    	<input type="file" name="animal_image[]" id="inputImage_{{$num_row}}" onchange="preview_Images_manage_cow(this);" class="hideme">
								    </label>
								</div>
						    </div>
					  	</div>
